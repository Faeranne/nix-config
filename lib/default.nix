inputs: let
  ...
  */
  getWireguardHost = wireName: let
    host =
      foldlAttrs (
        acc: name: value:
          if (builtins.hasAttr "wg${wireName}" value.config.networking.wireguard.interfaces)
          then name
          else acc
      ) ""
      self.nixosConfigurations;
  in
    assert assertMsg (host != "") "Could not find wg${wireName} in any host"; host;
  /*
  *
  ...
  */
  gatherContainers = (
    concatMapAttrs (
      host: hostInstance: let
        hostConfig = hostInstance.config;
        hostWireguards = hostConfig.networking.wireguard.interfaces;
        agePath = hostConfig.age.secretsDir + "/";
      in
        mapAttrs (container: containerInstance: let
          specialArgs = containerInstance.specialArgs;
          wg = hostWireguards."wg${container}";
          ports =
            if (builtins.hasAttr "ports" specialArgs)
            then specialArgs.ports
            else {${container} = specialArgs.port;};
          hostNames =
            if (builtins.hasAttr "hostNames" specialArgs)
            then specialArgs.hostNames
            else {${container} = specialArgs.hostName;};
