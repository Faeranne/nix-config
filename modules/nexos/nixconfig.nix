...
in {
    ...
    nexos = {
      nixpkgs = {
        ...
        repo = {
          base = mkOption {
            type = str;
            default = "git+https://git.faeranne.com/faeranne/nix-config";
            description = "Base repo for this set of systems.";
          };
          tag = mkOption {
            type = str;
            default = "rebuild2";
  ...
  config = mkIf enable {
    system = {
      autoUpgrade = mkIf config.nexos.networking.online {
        operation = "switch";
        flake = "${cfg.repo.base}?ref=${cfg.repo.tag}";
      };
    };
    nix = {
      settings = {
    ...
    age = {
      secrets = {
        ...
        flake-accessTokens = {
          rekeyFile = secrets + "/accessTokens.age";
          mode = "770";
          group = "nixbld";
        };
      };
